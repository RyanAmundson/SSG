...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $sql ="SELECT
  (SELECT COUNT(*) FROM Issues) as table1Count,
  (SELECT COUNT(*) FROM LawsAgreements) as table2Count,
  (SELECT COUNT(*) FROM PolicyActors) as table3Count;";

    $result = mysqli_query($con,$sql) or die(mysqli_error($con));
    $row = mysqli_fetch_assoc($result);
    $resultArray[0] = $row["table1Count"];
    $resultArray[1] = $row["table2Count"];
    $resultArray[2] = $row["table3Count"];
